...
if "--testWF" in ARGS
    ...
    psi = psi_n_batch(wf, xi, UInt(4), 0.)
    println(sum(psi.*wi))
    plot(r, real.(psi_n_batch(wf, r, UInt(1), 0.)), dpi=300)
    plot!(r, real.(psi_n_batch(wf, r, UInt(2), 0.)), dpi=300)
    plot!(r, real.(psi_n_batch(wf, r, UInt(3), 0.)), dpi=300)
    plot!(r, real.(psi_n_batch(wf, r, UInt(4), 0.)), dpi=300)
    plot!(r, real.(psi_n_batch(wf, r, UInt(5), 0.)), dpi=300)
    savefig("psi.png")
end
...
if "--testPsi" in ARGS
    ...
    # psi = lse_get_psi(lse)
    data = read("wf.bin")
    psi = reinterpret(ComplexF64, data)
    psi = transpose(reshape(psi, (1000, 30)))
    plot(range(0,7,1000), real.(psi[1, :]), dpi=300)
    plot!(range(0,7,1000), real.(psi[2, :]))
    plot!(range(0,7,1000), real.(psi[3, :]))
    plot!(range(0,7,1000), real.(psi[4, :]))
    plot!(range(0,7,1000), real.(psi[5, :]))
    # xlims!(1, 5)
    savefig("psi.png")
    # for i in 1:30
    #     local quad = 0
