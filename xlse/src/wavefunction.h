...
static inline double r_n(int n) { return R_1 * pow(R_N_MAX / R_1, ((double)(n - 1) / (N_MAX - 1))); }
...
{
      return sqrt(pow(2, l + 2) * pow(2 * nu_n(n), l + 1.5) / (sqrt(PI) * doublefactorial(2 * l + 1)));
}
...
{
      return 1.0 / sqrt(2 * (nu_n(n) + nu_n(n_prime))) * doublefactorial(2 * l + 2) / doublefactorial(2 * l + 1) *
	     N_n_n_prime(n, n_prime, l);
}

static inline double V_1_r_n_n_prime(int n, int n_prime, int l)
{
      return 2.0 / sqrt(PI) * pow(2, l) * gsl_sf_fact(l) / doublefactorial(2 * l + 1) * sqrt(nu_n(n) + nu_n(n_prime)) *
	     N_n_n_prime(n, n_prime, l);
}
...
{
      return sqrt(pow(2, l + 3) * pow(2 * nu_n(n), l + 2.5) / (sqrt(PI) * doublefactorial(2 * l + 3)));
}
...
{
      return 1.0 / sqrt(2 * (nu_n(n) + nu_n(n_prime))) * doublefactorial(2 * l + 4) / doublefactorial(2 * l + 3) *
