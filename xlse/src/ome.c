...
double complex Vpiu(struct OME ome, _Complex double E, _Complex double p1,
                    _Complex double p2, double complex m1, double complex m2,
                    double complex m3, double complex m4, double m0,
                    double fac) {
    double frame = 0.3;
    auto _z0 = z0(E, m1, m3, p1, p2, m0);
    ...
    if (fabs(creal(_z0)) > 1 || fabs(cimag(_z0)) > frame) {
        if (fabs(creal(_z0E)) > 1 || fabs(cimag(_z0E)) > frame) {
            res += quadreal(ome, E, p1, p2, m1, m3, m0, fac);
        } else if (cimag(_z0E) > 0) {
            res += quaddn(ome, E, p1, p2, m1, m3, m0, fac);
        } else {
            res += quadup(ome, E, p1, p2, m1, m3, m0, fac);
        }
    } else if (cimag(_z0) > 0) {
        if (fabs(creal(_z0E)) > 1 || fabs(cimag(_z0E)) > frame ||
            cimag(_z0E) > 0) {
            ...
            res += quadii(ome, E, p1, p2, m1, m3, m0, fac, _z01 + 1, -1) +
                   quadii(ome, E, p1, p2, m1, m3, m0, fac, _z02 - _z01, _z01) +
                   quadii(ome, E, p1, p2, m1, m3, m0, fac, 1 - _z02, _z02);
        }
    } else {
        if (fabs(creal(_z0E)) > 1 || fabs(cimag(_z0E)) > frame ||
            cimag(_z0E) < 0) {
    ...
    _z0 = z0(E, m2, m4, p1, p2, m0);
#ifdef DEBUG
    printf("Vpiu(z0) : %s\n", formatC(_z0));
#endif
    if (fabs(creal(_z0)) > 1 || fabs(cimag(_z0)) > frame) {
        if (fabs(creal(_z0E)) > 1 || fabs(cimag(_z0E)) > frame) {
            res += quadreal(ome, E, p1, p2, m2, m4, m0, fac);
        } else if (cimag(_z0E) > 0) {
            res += quaddn(ome, E, p1, p2, m2, m4, m0, fac);
        } else {
            res += quadup(ome, E, p1, p2, m2, m4, m0, fac);
        }
    } else if (cimag(_z0) > 0) {
        if (fabs(creal(_z0E)) > 1 || fabs(cimag(_z0E)) > frame ||
            cimag(_z0E) > 0) {
            ...
            res += quadii(ome, E, p1, p2, m2, m4, m0, fac, _z01 + 1, -1) +
                   quadii(ome, E, p1, p2, m2, m4, m0, fac, _z02 - _z01, _z01) +
                   quadii(ome, E, p1, p2, m2, m4, m0, fac, 1 - _z02, _z02);
        }
    } else {
        if (fabs(creal(_z0E)) > 1 || fabs(cimag(_z0E)) > frame ||
            cimag(_z0E) < 0) {
...
double complex quad(double complex E, double complex p1, double complex p2) {
    ...
    for (size_t i = 0; i < DIMRE; i += 1) {
        ...
        auto Dint = FACPI * (1 / D1 + 1 / D2) / (2 * Epi(z, p1, p2, m0)) *
                    (p1 * p1 + p2 * p2 - 2 * p1 * p2 * z);
        // auto Dint = ( 1/D1 + 1 / D2) / (2*Epi(z, p1, p2, m0)) * (z);
        // auto Dint = (1 / D1 + 1 / D2) / (2 * Epi(z, p1, p2, m0)) * (- 2 * p1
        // * p2 * z);
        res += Dint * w;
    }
...
double complex juliana(double complex E, double complex p,
    ...
    // 2*p*pprime*Delta1_00(E, p, pprime, m_pi); return -Delta1_00(E, p, pprime,
    // m_pi);
