...
int lse_tmat_single(LSE *self)
{
    ...
    gsl_complex alpha = gsl_complex_rect(1.0, 0.0);
    gsl_complex beta = gsl_complex_rect(0.0, 0.0);
    gsl_blas_zgemm(CblasNoTrans, CblasNoTrans, alpha, &V.matrix, &G.matrix, beta, VG);
    gsl_matrix_complex_scale(VG, gsl_complex_rect(-1.0, 0.0)); // I_minus_VG = -VG
...
void lse_X(LSE *self)
{
	...
	for (size_t beta = 0; beta < 2; beta += 1) {
		...
		for (size_t pi = 0; pi < pNgauss + 1; pi += 1) {
			...
			double complex G = matrix_get(self->G, ki + alpha * (pNgauss + 1), ki + alpha * (pNgauss + 1));
			double complex T = matrix_get(self->TOME, ki + alpha * (pNgauss + 1), pi + beta * (pNgauss + 1));
...
void lse_XtX(LSE *self)
{
	...
	for (size_t beta = 0; beta < 2; beta += 1) {
		...
		for (size_t delta = 0; delta < 2; delta += 1) {
			...
			for (size_t j = 0; j < N_MAX + 1; j += 1) {
			    T[alpha][beta] +=
				Xout[alpha][gamma][i][pNgauss] * invMv[gamma][delta][i][j] * (Xout[delta][beta][j][pNgauss]);
			}
		    }
		}
	    }
