...
void testwf() {
  ...
  //   puts("");
  // }
  LSE *lse __attribute__((cleanup(auto_lsefree))) = lse_malloc(64, 4, 1e-6);
  FILE *data __attribute__((cleanup(auto_fclose))) = fopen("psi.dat", "wb");
  FILE *energy __attribute__((cleanup(auto_fclose))) = fopen("energy.dat", "wb");
  fwrite(lse->psi_raw, sizeof(double complex), N_MAX * (64 + 2), data);
  fwrite(lse->wf->E_solution->data, sizeof(double), N_MAX, energy);
}
