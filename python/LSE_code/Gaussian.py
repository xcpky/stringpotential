...
def _():
    ...
    import math
    return fft, fftfreq, figure, ifft, math, njit, np, plt, scipy, special
...
def _(math, np, special):
    ...
    def N_nl(n, l):
        return np.sqrt(
            2 ** (l + 2)
            * (2 * nu_n(n)) ** (l + 3 / 2)
            / (np.sqrt(np.pi) * doublefactorial(2 * l + 1))
        )


    def N_n_n_prime(n, n_prime, l):
        return np.power(
            2 * np.sqrt(nu_n(n) * nu_n(n_prime)) / (nu_n(n) + nu_n(n_prime)),
            l + 3 / 2,
        )
    ...
    def T_n_n_prime_test(n, n_prime, l):
        return (
            2 * l * (l - 1) / (2 * l + 1) * (nu_n(n) + nu_n(n_prime))
            - 2 * nu_n(n_prime) * (l + 2)
            + 2 * nu_n(n_prime) ** 2 * (2 * l + 3) / (nu_n(n) + nu_n(n_prime))
        ) * N_n_n_prime(n, n_prime, l)
    ...
    def V_r_n_n_prime(n, n_prime, l):
        return (
            1
            / (np.sqrt(2 * (nu_n(n) + nu_n(n_prime))))
            * doublefactorial(2 * l + 2)
    ...
    def N_nl_tilde(n, l):
        return np.sqrt(
            2 ** (l + 3)
            * (2 * nu_n(n)) ** (l + 5 / 2)
            / (np.sqrt(np.pi) * doublefactorial(2 * l + 3))
        )
    ...
    def T_n_n_prime_tilde_test(n, n_prime, l):
        return (
            2
            * ((2 * l + 3) * nu_n(n) * nu_n(n_prime) - 2 * nu_n(n_prime) ** 2)
            / (nu_n(n) + nu_n(n_prime))
            - np.sqrt(2 * (nu_n(n) + nu_n(n_prime)))
            * l
            * (l + 1)
            * doublefactorial(2 * l + 2)
            / doublefactorial(2 * l + 3)
    ...
    def T_n_n_prime_tilde(n, n_prime, l):
        return (
            c_T
            * (
                (2 * l + 5) * nu_n(n) * nu_n(n_prime) / (nu_n(n) + nu_n(n_prime))
                - (l + 1)
            )
            * N_n_n_prime_tilde(n, n_prime, l)
        )
    ...
    def V_e_mu_n_n_prime(n, n_prime, l, mu):
        return np.power(
            2 * np.sqrt(nu_n(n) * nu_n(n_prime)) / (nu_n(n) + nu_n(n_prime) + mu),
            l + 3 / 2,
        )
    ...
    def V_e_mu_lin_n_n_prime_l_0(n, n_prime, mu):
        return (
            np.sqrt(4 * (2 * nu_n(n)) ** (3 / 2) / (np.sqrt(np.pi)))
            * np.sqrt(4 * (2 * nu_n(n_prime)) ** (3 / 2) / (np.sqrt(np.pi)))
            * (
                1 / (2 * (nu_n(n) + nu_n(n_prime)))
                - np.sqrt(np.pi)
                * mu
                * special.erfcx(mu / (2 * np.sqrt(nu_n(n) + nu_n(n_prime))))
                / (4 * (nu_n(n) + nu_n(n_prime)) ** (3 / 2))
            )
        )


    def V_deuteron_n_n_prime(n, n_prime):
        return -626.885 * V_e_mu_lin_n_n_prime_l_0(
            n, n_prime, 1.55
...
def _(E_solution, np):
    np.savetxt(
        "E_solution_GaussBasis_Cornell_30_V0_04.txt", E_solution, fmt="%.8e"
    )
    return
...
def _(c_solution, np):
    np.savetxt(
        "c_solution_GaussBasis_Cornell_30_V0_04.txt", c_solution, fmt="%.8e"
    )
    return
...
def _(E_solution, a_Cornell, m_c):
    print(
        "In dimensionless units, \zeta:",
        m_c ** (-1) * (m_c * a_Cornell) ** (4 / 3) * E_solution,
    )
    return


@app.cell
def _(a_Cornell, c_solution, m_c):
    print(c_solution[:, 0])
    print(m_c ** (-1) * (m_c * a_Cornell) ** (4 / 3))
    print(m_c)
    return
...
def _(N_nl, N_nl_tilde, c_solution, n_max, np, nu_n, special):
    def psi_nlm(r, n, l, m, theta, phi):
        ...
        for _i in range(n_max):
            _n_prime = _i + 1
            psi = psi + c_solution[:, n][_i] * N_nl(_n_prime, _l) * r**_l * np.exp(
                -nu_n(_n_prime) * r**2
            ) * special.sph_harm(m, _l, theta, phi)
        return psi
    ...
    def psi_nlm_tilde(r, n, l, m, theta, phi):
        ...
        for _i in range(n_max):
            _n_prime = _i + 1
            psi = psi + c_solution[:, n][_i] * N_nl_tilde(_n_prime, _l) * r ** (
                _l + 1
            ) * np.exp(-nu_n(_n_prime) * r**2) * special.sph_harm(
                m, _l, theta, phi
            )
        return psi
...
def _(c_solution, n_max, np, nu_n, psi_nlm):
    def N_nl_p(n_prime):
        return np.sqrt(
            4 * np.sqrt(2) / (9 * np.pi ** (7 / 2)) * nu_n(_n_prime) ** (5 / 2)
        )
    ...
    def psi_nlm_p(p, n):
        ...
        for _i in range(n_max):
            _n_prime = _i + 1
            psi_p = psi_p + c_solution[:, n][_i] * N_nl_p(_n_prime) * np.sqrt(
                3
            ) * np.pi * 1j * 1 / (4 * nu_n(_n_prime) ** (5 / 2)) * p * np.exp(
                -(p**2) / (4 * nu_n(_n_prime))
            )
...
def _(np, plt, psi_nlm_cs, special):
    ...
    plt.plot(
        r_plot,
        r_plot
        * psi_nlm_cs(r_plot, 0, ll, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot,
        r_plot
        * psi_nlm_cs(r_plot, 1, ll, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot,
        r_plot
        * psi_nlm_cs(r_plot, 2, ll, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot,
        r_plot
        * psi_nlm_cs(r_plot, 3, ll, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot,
        r_plot
        * psi_nlm_cs(r_plot, 4, ll, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot,
        r_plot
        * psi_nlm_cs(r_plot, 5, ll, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot,
        r_plot
        * psi_nlm_cs(r_plot, 6, ll, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.grid()
    return ll, r_plot
...
def _(
    ...
    yu0 = (
        x_interpolate
        * psi_nlm_cs(x_interpolate, 0, 0, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real
        + 0.02
    )
    yd0 = (
    ...
    yu1 = (
        x_interpolate
        * psi_nlm_cs(x_interpolate, 1, 0, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real
        + 0.02
    )
    yd1 = (
    ...
    yu2 = (
        x_interpolate
        * psi_nlm_cs(x_interpolate, 2, 0, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real
        + 0.02
    )
    yd2 = (
    ...
    yu3 = (
        x_interpolate
        * psi_nlm_cs(x_interpolate, 3, 0, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real
        + 0.02
    )
    yd3 = (
...
def _(np, plt, psi_nlm_cs, special):
    ...
    plt.plot(
        r_plot_1,
        r_plot_1
        * psi_nlm_cs(r_plot_1, 0, _l, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot_1,
        r_plot_1
        * psi_nlm_cs(r_plot_1, 1, _l, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot_1,
        r_plot_1
        * psi_nlm_cs(r_plot_1, 2, _l, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot_1,
        r_plot_1
        * psi_nlm_cs(r_plot_1, 3, _l, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot_1,
        r_plot_1
        * psi_nlm_cs(r_plot_1, 4, _l, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot_1,
        r_plot_1
        * psi_nlm_cs(r_plot_1, 5, _l, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.plot(
        r_plot_1,
        r_plot_1
        * psi_nlm_cs(r_plot_1, 6, _l, 0, 0, 0).real
        / special.sph_harm(0, 0, 0, 0).real,
    )
    plt.grid()
    return (r_plot_1,)
...
def _(np):
    def PIgauss(x, a1, a2, mu1, mu2, sigma1, sigma2):
        return a1 * np.exp(-((_x - mu1) ** 2) / (2 * sigma1**2)) + a2 * np.exp(
            -((_x - mu2) ** 2) / (2 * sigma2**2)
        )
    return (PIgauss,)
...
def _(E_solution, plt, psi_nlm, r_plot_1):
    ...
    _axs[0].text(
        4, 0.19, "$E_0=%.2f \\,\\mathrm{GeV}$" % E_solution[0], fontsize=15
    )
...
def _(m11, m12, m21, m22, np):
    ...
    def q_0_func(E):
        q0 = np.array(
            [
                np.sqrt(2 * mu[0] * (E - Delta[0]) + 0j),
                np.sqrt(2 * mu[1] * (E - Delta[1]) + 0j),
            ]
        )
        return q0.real + 0j
...
def _(Gauss_Legendre, Gauss_herm, Gauss_herm_mesh, Gauss_mesh, np, psi_nlm_cs):
    ...
    def Ftpsi_3P1(p, n, N_gauss, Lambda):
        ...
        result1 = (
            ...
            * (2 * np.pi)
            * (
                Gauss_Legendre(integrand_3P1, 0, Lambda, _x, _w, p, n)
                + Gauss_Legendre(integrand_3P1, 0, Lambda, _x, _w, -p, n)
            )
        )
    ...
    def Ftpsi_S_1_3(p, n, N_gauss, Lambda):
        ...
        result1 = (
            ...
            * (2 * np.pi)
            * (
                Gauss_Legendre(integrand_3S1, 0, Lambda, _x, _w, p, n)
                + Gauss_Legendre(integrand_3S1, 0, Lambda, _x, _w, -p, n)
            )
        )
        result2 = result1.real**2 + result1.imag**2
    ...
    def Ftpsi_S_1_3_hermite(p, n, N_gauss):
        ...
        result1 = (
            ...
            * (2 * np.pi)
            * (
                Gauss_herm(integrand_3S1, _x, _w, p, n)
                + Gauss_herm(integrand_3S1, _x, _w, -p, n)
            )
        )
        result2 = result1.real**2 + result1.imag**2
...
def _(
    ...
    for _i in range(Ntower):
        ...
        for _l in range(len(E_array)):
            psi_mat_q1[_i + Ntower * _l, Nq] = Ftpsi_S_1_3(
                q_0_func(E_array[_l])[0], _i, Nq, 20.0
            )[0].real
            psi_mat_q2[_i + Ntower * _l, Nq] = Ftpsi_S_1_3(
                q_0_func(E_array[_l])[1], _i, Nq, 20.0
            )[0].real
...
def _(
    ...
    for _i in range(Ntower):
        ...
        for _k in range(psi_mat_q1.shape[1] - 1):
            ...
            psi_mat_q2[_i, _k] = psi_mat_q1[_i, _k]
        for _l in range(len(E_array)):
            psi_mat_q1[_i + Ntower * _l, Nq] = psi_nlm_p(
                q_0_func(E_array[_l])[0], _i
            ).real
            psi_mat_q2[_i + Ntower * _l, Nq] = psi_nlm_p(
                q_0_func(E_array[_l])[1], _i
            ).real
            psi_mat_q1[_i + Ntower * _l, :-1] = psi_mat_q1[_i, :-1]
            psi_mat_q2[_i + Ntower * _l, :-1] = psi_mat_q2[_i, :-1]
    return (state_2,)
...
def _(integrand, np, rectangular_int):
    def Ftpsi_S_1_3_rec(p, n, N, Lambda):
        result1 = (
            ...
            * (2 * np.pi)
            * (
                rectangular_int(integrand, 0, Lambda, N, p, n)
                + rectangular_int(integrand, 0, Lambda, N, -p, n)
            )
        )
        result2 = result1.real**2 + result1.imag**2
...
def _(N_nl, N_nl_p, np, plot_p_1, plot_psi_p_40_0):
    ...
    for _i in range(plot_p_1.size):
        N_test[_i] = (
            plot_p_1[_i] ** 2
            * np.abs(N_nl_p(0) / N_nl(0, 0) * plot_psi_p_40_0[_i]) ** 2
            * 20
            / 1000
        )
    print(np.sum(N_test))
...
def _(np, plot_p_1, plt, psi_nlm_cs, psi_nlm_p):
    ...
    _axs[0].plot(
        plot_p_1,
        np.abs(psi_nlm_p(plot_p_1, 0)) ** 2,
        label="$|\\psi_{0}|^2$, analytical",
        color="darkblue",
        linestyle="dashed",
    )
    _axs[0].plot(
        plot_p_1,
        np.abs(psi_nlm_p(plot_p_1, 1)) ** 2,
        label="$|\\psi_{1}|^2$, analytical",
        color="darkorange",
        linestyle="dashed",
    )
    _axs[0].plot(
        plot_p_1,
        np.abs(psi_nlm_p(plot_p_1, 2)) ** 2,
        label="$|\\psi_{2}|^2$, analytical",
        color="darkgreen",
        linestyle="dashed",
    )
    _axs[0].set_ylabel(
        "$\\psi_{i,l=1}(p)\\,/\\,\\mathrm{GeV}^{-3/2}$", fontsize=20
    )
...
def _(Gauss_Legendre, Gauss_mesh, np, test_r):
    ...
    def ftTest(p, f, N_gauss, Lambda):
        _x, _w = Gauss_mesh(N_gauss)
        result1 = -1j * (
            Gauss_Legendre(f, 0, Lambda, _x, _w, p)
            + Gauss_Legendre(f, 0, Lambda, _x, _w, -p)
        )
        result2 = abs(result1)
        return (result1, result2)
